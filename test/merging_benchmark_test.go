...
func BenchmarkMergeData(b *testing.B) {
	m := merging.New()
	...
	assert.Equal(b, 25, h.IDHotel)
	assert.Equal(b, 1034, h.DestinationID)
	assert.Equal(b, "Hotel California", h.Name)
	assert.Equal(b, 3, len(h.Destinations))
}

func BenchmarkMergeDataWithWishlistAndBlacklist(b *testing.B) {
	m := merging.New()
